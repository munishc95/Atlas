...
export function AppShell({ children }: { children: React.ReactNode }) {
    ...
    mutationFn: async () =>
      (
        await atlasApi.runBacktest({
          ...
          params: { trend_period: 200, breakout_lookback: 20, atr_stop_mult: 2.0, atr_trail_mult: 2.0 },
          config: {},
        })
      ).data,
    onSuccess: (data) => {
  ...
  return (
    <div className="min-h-screen text-ink">
      <CommandPalette open={paletteOpen} commands={commands} onClose={() => setPaletteOpen(false)} onSelect={runCommand} />
      ...
      <div className="mx-auto flex w-full max-w-[1440px] gap-4 p-4 md:p-6">
        ...
        <div className="min-w-0 flex-1">
          <header className="card mb-4 flex items-center justify-between px-4 py-3 md:px-5">
            <div>
              <p className="text-xs uppercase tracking-[0.1em] text-muted">Research + Paper Trading</p>
